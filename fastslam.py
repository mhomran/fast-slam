    ...
    def correction_step(self, sensor_measurements):
        ...
        for measurement in sensor_measurements:
            ...
            if not landmark.observed:
                ...
                phi = measurement.z_bearing
                x, y, theta = robot_pose
                landmark.mu[0] = x + r * np.cos(phi+theta)
                landmark.mu[1] = y + r * np.sin(phi+theta)
                # get the Jacobian
                ...
                error = np.zeros_like(expected_z)
                error[0] = measurement.z_range - expected_z[0]
                error[1] = normalize_angle(measurement.z_bearing - expected_z[1])
                # TODO: update the mean and covariance of the EKF for this landmark
                landmark.mu = landmark.mu + K @ error
...
def main():
    ...
    anim = animation.FuncAnimation(plt.gcf(), plotter.fast_slam, 
    frames=np.arange(0, len(data)), init_func=plotter.plot_state_init, 
